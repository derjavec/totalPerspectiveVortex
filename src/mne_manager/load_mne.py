...
def load_raw_eeg(subject, runs):
    """Load and concatenate EEGBCI runs for a subject with consistent MNE config."""
    ...
    mne_data_dir = external_mne_data_dir if os.path.isdir(external_mne_data_dir) else default_mne_data_dir
    ...
    files = eegbci.load_data(subject, runs, path=mne_data_dir, update_path=False)
